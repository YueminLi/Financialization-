...
po <- read.csv("table_and_figure/phillips-ouliaris.csv")
kbl(po, booktabs = T, digit = 3,
    caption = "Phillips-Ouliaris Tests for Cointegration", 
    col.names = rep(c("","statistics", "p-value", "statistics", "p-value", "statistics", "p-value" ))) %>%
  add_header_above(c("","Value Added" = 2, "NFC debts" = 2, "Household Debts" = 2)) 
...
cm2 <- c("(Intercept)" = "(Intercept)",
        ...
        "deltaC_REER" = "Real Effect Exchange Rates",
        "deltaC_wgdp" = "GDP",
        "deltaC_cpi" = "CPI",
        "deltaC_wgini" = "GINI")
...
modelsummary(model2,
         ...
         coef_map = cm2,
         fmt = 4,
         title = "Error Correction Models, 1972-2012") %>%
   pack_rows("Statecraft contigency", 3, 10) %>%
   pack_rows("Development pitfall", 11,18) %>%
   pack_rows("Control variable", 19,26) 
