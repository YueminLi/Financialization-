...
p_load(tidyverse, knitr, kableExtra, rmarkdown, ggplot2, modelsummary, gt)
```
...
#Table for adf and correlation
padf_table <- read.csv("./table_and_figure/padf_table.csv")
padf_table$X <- NULL
cor_mat <- read.csv("./table_and_figure/cor_mat.csv")
cor_mat$X <- NULL
t1 <- cbind(padf_table, cor_mat)
t1 <- t1[,c(-3)]

options(knitr.kable.NA = '')
kable(t1,  "pipe",
      digits = 2, booktabs = T,
      caption = "Augmented Dickey-Fuller Test for Unit Root",
      col.names = rep(c("Variable", "ADF Statistics",
                        "Values added in finance activities", "Nonfinancial corporate debts", "Household debts",
...
```{r echo = FALSE, rsult = 'asis'}
#table for Phillips-Ouliaris Test for Co-integration

t2 <- read.csv('./table_and_figure/po.csv')
t2$X <- NULL
kable(t2, "pipe",
      digit = 2, booktabs = T,
      caption = "Phillips-Ouliaris Test for Co-integration And Correlation", 
      col.names = rep(c("Phillips-Ouliaris demeaned","Values added in finance activities", 
                        "Nonfinancial corporate debts", "Household debts")))


```

```{r echo = FALSE, rsult = 'asis'}
hausman <- read.csv('./table_and_figure/hausman_test.csv')
hausman$X <- NULL
hausman$X.1 <- NULL
hausman$X.2 <- NULL
kable(hausman, "pipe",
      ...
      col.names = rep(c("Model", "Chi-square", "P-value", "Result"))
      )
```
