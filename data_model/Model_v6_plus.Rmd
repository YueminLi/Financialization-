...
df <- df %>%
      filter(Year >= 1960) 
colnames(df)
df$IV_fdi_net[df$IV_fdi_net==0] <- NA
```

Then create lagged variables and _pc variables.<br/>
...
dplyr::select(Country, Year, DV_VA_pc, DV_VA_pc_lag1,
              IV_lending_pc, IV_gov_exp_pc, IV_trade_balance_pc, IV_fdi_net_pc, IV_wgini_pc,
              IV_lending_pc_lag1, IV_gov_exp_pc_lag1,
              IV_trade_balance_pc_lag1, IV_fdi_net_pc_lag1,
...
po.test(as.matrix(cbind(va$DV_VA_pc, va$IV_fdi_net_pc), demean=FALSE))
...
po.test(as.matrix(cbind(nfc$DV_nfc_ls_pc, nfc$IV_fdi_net_pc), demean=FALSE))
